...
awk '!array_temp[$0]++' ~/.wallpaper_manager/tags/todos_favoritos > ~/.wallpaper_manager/tags/todos_favoritos1
cat ~/.wallpaper_manager/tags/todos_favoritos1 | sed '/./!d' | shuf > /tmp/buenas-imagenes2
while read LINE
do
    if [ -f "$LINE" ]
    then
...
echo ${DEBUG:+"Creando el Edit xml."}
...
rm ~/.wallpaper_manager/tags/todos_favoritos
rm ~/.wallpaper_manager/tags/todos_favoritos1
