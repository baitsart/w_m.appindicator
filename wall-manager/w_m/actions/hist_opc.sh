...
activ()
{
sh /usr/share/wallpaper_manager/w_m/actions/active-hist.sh
}
dialogo()
{
	...
	file=$( zenity --list --radiolist --width="150" --height="260" --title="wallpaper manager" \
					--column='Pick' --column='Option'  \
					'TRUE' 		"List_history" \
					'FALSE' 		"History_viewer" \
					'FALSE' 		"include_in_history" \
					'FALSE' 		"Edit_history_list" \
					'FALSE' 		"Active_deactive" \
					'FALSE' 		"Data_by_date" )
}
comandos()
{
	dialogo
	if [ ${file} = 'List_history' ]; then
		list
	elif [ ${file} = 'History_viewer' ]; then
		ver
	elif [ ${file} = 'include_in_history' ]; then
		ir_prev
	elif [ ${file} = 'Edit_history_list' ]; then
		edit
	elif [ ${file} = 'Active_deactive' ]; then
		activ
	elif [ ${file} = 'Data_by_date' ]; then
		dat
	else
		echo "No selection."
		exit 1
