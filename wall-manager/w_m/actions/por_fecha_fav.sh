...
case $? in
         0)
		cat ~/.wallpaper_manager/tags/datos_fechados/buenas_imagenes_fechado | grep -n "$age" | cut -d':' -f1 > /tmp/temporal-line-numbers
sed -i '/^$/d' /tmp/temporal-line-numbers
cat /tmp/temporal-line-numbers | while read line ; do linea=$(echo $line); sed ''$linea'!d' ~/.wallpaper_manager/tags/datos_fechados/buenas_imagenes_fechado | cat >> /tmp/temporal-line-numbers1; done
comands
exit;;
         1)
                exit 1;;
...
case $? in
         0)
		cat ~/.wallpaper_manager/tags/datos_fechados/buenas_imagenes_fechado | cut -c 7-8 | grep -n "$age" | cut -d':' -f1 > /tmp/temporal-line-numbers
sed -i '/^$/d' /tmp/temporal-line-numbers
cat /tmp/temporal-line-numbers | while read line ; do linea=$(echo $line); sed ''$linea'!d' ~/.wallpaper_manager/tags/datos_fechados/buenas_imagenes_fechado | cat >> /tmp/temporal-line-numbers1; done
comands
exit;;
...
case $? in
         0)
		cat ~/.wallpaper_manager/tags/datos_fechados/buenas_imagenes_fechado | cut -c 4-8 | grep -n "$age" | cut -d':' -f1 > /tmp/temporal-line-numbers
sed -i '/^$/d' /tmp/temporal-line-numbers
cat /tmp/temporal-line-numbers | while read line ; do linea=$(echo $line); sed ''$linea'!d' ~/.wallpaper_manager/tags/datos_fechados/buenas_imagenes_fechado | cat >> /tmp/temporal-line-numbers1; done
comands
exit;;
...
then
sed -i 's|'"$file"'||' ~/.wallpaper_manager/tags/datos_fechados/buenas_imagenes_fechado
...
options()
{
	...
	file=$( zenity --list --radiolist --width="295" --height="215" --title="Dated favorites" --text="Choose an action" \
					--column='Pick' --column='Option'  \
					'FALSE' 		"View_dated_favorites" \
					'TRUE' 		"Slideshow_dated_favorites" \
					'FALSE' 		"List_dated_favorites" \
					'FALSE' 		"Xml_dated_favorites" )
}
comands()
{
	options
	if [ ${file} = 'View_dated_favorites' ]; then
		ver
	elif [ ${file} = 'Slideshow_dated_favorites' ]; then
		slide
	elif [ ${file} = 'List_dated_favorites' ]; then
		listar
	elif [ ${file} = 'Xml_dated_favorites' ]; then
		xmls
	else
		echo "No selection."
		exit 1
...
dialogo()
{
	...
	file=$( zenity --list --radiolist --width="150" --height="190" --title="Dated favorites" --text="Choose the time range" \
					--column='Pick' --column='Option'  \
					'FALSE' 		"Of_a_day" \
					'TRUE' 		"Of_a_month" \
					'FALSE' 		"Of_a_year" )
