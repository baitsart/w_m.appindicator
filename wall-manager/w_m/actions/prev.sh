...
sed -i '/^$/d' /tmp/background_notify
var=$(cat /tmp/background_notify)
if
cat ~/.wallpaper_manager/historial | tail -n1 | grep -v "$var"
then
sh /usr/share/wallpaper_manager/w_m/actions/ir-prev.sh
fi

tac ~/.wallpaper_manager/historial | uniq > /tmp/background_notify1



tac /tmp/background_notify1 | grep -B1 "^$var" > /tmp/background_notify2

head -n 1 /tmp/background_notify2 > /tmp/background_notify
